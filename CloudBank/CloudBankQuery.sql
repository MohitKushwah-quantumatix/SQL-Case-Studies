...
select distinct node_id as dictinct_node_ID,
count(node_id) as count_per_node
from customer_nodes
group by node_id
order by node_id;

--2. What is the distribution of nodes across different regions?
select n.node_id, r.region_id, r.region_name, count(n.node_id) as distribution_of_nodes_across_regions
from regions r inner join customer_nodes n
on r.region_id = n.region_id
group by n.node_id, r.region_id, r.region_name;

--3. How many customers are allocated to each region?
select r.region_id, r.region_name, count(n.customer_id) as customer_count_per_region
from regions r inner join customer_nodes n
on r.region_id = n.region_id
...
with cte as (
select customer_id ,node_id, start_date, end_date ,datediff(DAY,start_date, end_date) as date_difference,
lead(start_date) over(partition by customer_id order by start_date) as reallocated_date
from customer_nodes
group by node_id, customer_id, start_date, end_date
)

select customer_id, node_id, start_date, end_date ,date_difference, reallocated_date,
avg(date_difference) over(partition by customer_id order by customer_id) as avg_days
from cte
where reallocated_date is not null;
...
lead(start_date)over(partition by customer_id order by start_date)as reallocate_date from customer_nodes
)

select avg(datediff(day,start_date ,end_date)) as avgdays_onnode from cte where reallocate_date is not null;
...
cte2 as(
select cte.customer_id, cte.node_id, cte.start_date, cte.end_date ,cte.date_difference, cte.reallocated_date,
avg(cte.date_difference) over(partition by cte.customer_id order by cte.customer_id) as avg_days
from cte
where cte.reallocated_date is not null
)

select avg(cte2.avg_days) from cte2;

--5. What are the median, 80th percentile, and 95th percentile reallocation durations (in days) for customers in each region?
with cte as(
select r.region_id, r.region_name ,n.start_date, n.end_date,
datediff(day, start_date, end_date) as reallocation_duration
from customer_nodes n inner join regions r                             --INCOMPLETE
on n.region_id = r.region_id
),

cte2 as (
select region_id, region_name, start_date, end_date, reallocation_duration,
cume_dist() over(order by region_id) as ranking
from cte
)

select region_name, reallocation_duration
from cte2
where
	case 
		when ranking = 50 then ranking;


--B.
--1. What are the unique counts and total amounts for each transaction type (e.g., deposit, withdrawal, purchase)?
select txn_type , count(distinct customer_id) as unique_customers ,sum(txn_amount) as total_sum
...
select txn_type, 
case
	when txn_type = 'deposit' then sum( distinct txn_amount)
	when txn_type = 'withdrawal' then sum( distinct txn_amount)    -- remove distinct to get previous answer
	when txn_type = 'purchase' then sum(distinct txn_amount)
	end as total_of_txn_type
from customer_transactions
group by txn_type;

--2. What is the average number of historical deposits per customer, along with the average total deposit amount?
with cte as (
select customer_id, txn_type, avg(txn_amount) as per_customer_deposit
from customer_transactions
...
cte2 as (
select avg(txn_amount) as total_avg_deposit
from customer_transactions
where txn_type = 'deposit'
)

select customer_id, txn_type, per_customer_deposit ,total_avg_deposit  from cte, cte2 order by customer_id
...
with deposit_count as (
		select MONTH(txn_date) as months,customer_id,count(txn_type) num_of_transitions  from customer_transactions
		where txn_type='deposit'
...
select d.months,COUNT(d.customer_id) as no_of_customers 
from deposit_count as d
join purchase_withdraw_count pw 
on d.months=pw.months 

and d.customer_id=pw.customer_id
...
with deposit_cte as (
select d.customer_id ,d.txn_type, count(d.txn_type) as count_of_deposits ,datename(MONTH, d.txn_date) as months
from customer_transactions d
group by customer_id ,txn_type, datename(MONTH, txn_date)
having d.txn_type = 'deposit' and count(d.txn_type) > 1
),
--select * from deposit_cte order by customer_id;

purchase_withdrawal_cte as (
select d.customer_id ,p.txn_type, count(p.txn_type) as count_of_purchase_withdrawal ,datename(MONTH, p.txn_date) as months
from customer_transactions p join deposit_cte d
on d.customer_id = p.customer_id
group by d.customer_id ,p.txn_type, datename(MONTH, txn_date)
having p.txn_type in ( 'withdrawal' , 'purchase') and count(p.txn_type) = 1
)
--select * from purchase_withdrawal_cte order by customer_id;

select count(d.customer_id)  as count_of_customers , d.months from deposit_cte d join purchase_withdrawal_cte p
on d.months = p.months and d.customer_id = p.customer_id
group by  d.months;  
...
deposit_cte as (
select customer_id, MONTH(txn_date) as months, isnull(sum(d.txn_amount), 0) as deposit_balance
from customer_transactions d
where d.txn_type = 'deposit'
group by d.customer_id, MONTH(txn_date)
--order by d.customer_id
),
--select * from deposit_cte order by customer_id, months
purchase_withdraw_cte as (
select customer_id, MONTH(txn_date) as months, isnull(sum(p.txn_amount), 0) as purchase_withdraw_balance
from customer_transactions p
where p.txn_type in( 'purchase', 'withdrawal')
group by p.customer_id, MONTH(txn_date)
--order by p.customer_id
),
--select * from purchase_withdraw_cte;
closing_bal_cte as (
select m.customer_id, m.months, d.deposit_balance, p.purchase_withdraw_balance,
isnull(d.deposit_balance,0) - isnull(p.purchase_withdraw_balance ,0) as closingBalance
from main_table m left join deposit_cte d
on m.customer_id = d.customer_id and m.months = d.months
left join purchase_withdraw_cte p
on m.months = p.months and m.customer_id = p.customer_id
),

final_cte as (
select customer_id, months, deposit_balance, purchase_withdraw_balance, ClosingBalance,
sum(closingBalance) over(partition by customer_id order by months rows between unbounded preceding and current row) as ClosingBal
from closing_bal_cte 
)

select customer_id, months, deposit_balance, purchase_withdraw_balance,ClosingBal from final_cte  order by customer_id, months;


--5. What percentage of customers increased their closing balance by more than 5% month-over-month?   5/100 * amount
with main_table as (
select customer_id, MONTH(txn_date) as months
from customer_transactions
group by customer_id, MONTH(txn_date)
...
--select * from customer_count order by customer_id;
five_percent as (
select customer_id, months,deposit_balance, purchase_withdraw_balance, ClosingBal,
abs((5 * ClosingBal) / 100 ) as five_percent
from final_cte
),
--select * from five_percent;


comparison as (
select customer_id, months,deposit_balance, purchase_withdraw_balance, ClosingBal, five_percent,
case 
	when ( lag(ClosingBal) over(partition by customer_id order by months) + lag(five_percent) over(partition by customer_id order by months) ) < ClosingBal then customer_id
	else null
	end as comparison 
from five_percent
),
...
cte_a as (
select customer_id ,count(customer_id) as actual_customer_count
from comparison
group by customer_id
),
--select * from cte_a;
cte_b as (
select customer_id ,count(comparison) as main_customer_count
from comparison
group by customer_id
),
--select * from cte_b
cte_a_b_minus as (
select c.customer_id  ,months,deposit_balance, purchase_withdraw_balance, ClosingBal, five_percent,comparison,actual_customer_count, main_customer_count
from cte_a join cte_b 
on cte_a.customer_id = cte_b.customer_id
join comparison c
on c.customer_id = cte_a.customer_id
--where actual_customer_count = main_customer_count + 1   
where main_customer_count > 0 --main_customer_count > 1 if wanna include customers who has increased 5% in any month
group by c.customer_id, actual_customer_count, main_customer_count  ,months,deposit_balance, purchase_withdraw_balance, ClosingBal, five_percent,comparison
)

--select count(distinct customer_id) from cte_a_b_minus;

select ((cast(count(distinct customer_id) as float)/500) * 100) as percentage_of_customers from cte_a_b_minus;
...
with main_table as (
select customer_id, eomonth(txn_date) as months, txn_type, txn_date, txn_amount
from customer_transactions
group by customer_id, eomonth(txn_date), txn_type, txn_date, txn_amount
...
from main_table
--group by customer_id, txn_type, txn_date, txn_amount
),
--select * from running_bal

EOM_bal as (
select *, 
ROW_NUMBER() over (partition by customer_id order by months desc) as EOM
from running_bal
),
--select * from EOM_bal order by months --where eom = 1

min_max_avg as (
select * , min(running_balance) over(partition by customer_id order by txn_date) as min_running, 
max(running_balance) over(partition by customer_id order by txn_date) as max_running,
avg(running_balance) over(partition by customer_id order by txn_date) as avg_running
from EOM_bal
--where EOM = 1
--group by customer_id, txn_type, txn_date, txn_amount
)
select *, concat(avg_running, ' ', 'GB') as Storage from min_max_avg
...
final_cte as (
select customer_id, months, deposit_balance, purchase_withdraw_balance, ClosingBalance,
sum(closingBalance) over(partition by customer_id order by months rows between unbounded preceding and current row) as ClosingBal_EOM
from closing_bal_cte 
)

select customer_id, months, deposit_balance, purchase_withdraw_balance,ClosingBal_EOM from final_cte  order by customer_id, months;
